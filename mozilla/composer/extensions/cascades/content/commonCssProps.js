 ...
 *   Fabien Cazenave <kaze@kompozer.net>, bug fixing
 *
...
// * Callback when an URL textarea's value is changed
// it sets the corresponding property/value on the element elt
//   param XULElement elt
//   param String property
//   param String id
function ChangeURLOnInput(elt, property, id)
{
  if (!gDialog.selectedObject) return;
  var value = kUrlString + elt.value + ")";
...
//   the cache.
function RemoveImageFromCache(url) { // Kaze: added
  if (url != "" && url != "none") {
    try {
...
function onBgImageUrlChanged()
{
  ...
  if (value != "" && value != "none") {
    ...
    RemoveImageFromCache(value);
    value = kUrlString + MakeAbsoluteUrl(value) + ")";
  }
  if (id != null) {
...
function onOpacityScrollbarAttrModified(aEvent)
{
  if (aEvent.attrName == "curpos") {
    ...
    //~ if (v == 1)
    if (v <= 0) { // Kaze: transparent <=> opacity = 0 (not 1!)
      gDialog.opacityLabel.setAttribute("value", gDialog.bundle.getString("Transparent"));
      v = "0";
    } else
      gDialog.opacityLabel.setAttribute("value", v);    
    //~ AddStyleToElement(gDialog.selectedObject, "-moz-opacity", v);
    if (v >= 1) // Kaze: remove 'opacity' rule if its value is 1
      AddStyleToElement(gDialog.selectedObject, "-moz-opacity", "");
    else
      AddStyleToElement(gDialog.selectedObject, "-moz-opacity", v);
    SetModifiedFlagOnStylesheet();
  }
...
function InitBackgroundTabPanel()
{
  ...
  // <Kaze> update the opacity label value, too
  if (opacity == 0)
    gDialog.opacityLabel.setAttribute("value", gDialog.bundle.getString("Transparent"));
  else
    gDialog.opacityLabel.setAttribute("value", opacity);
  // </Kaze>
  ...
  var url = backgroundImage.replace(/^url\((.*)\)$/, "$1") // Kaze
  gDialog.backgroundImageInput.setAttribute("value", url); // Kaze
  //~ gDialog.backgroundImageInput.setAttribute("value", MakeRelativeUrl(url)); // Kaze
  enableBackgroundProps( (backgroundImage != "none" && backgroundImage != "") );
  RemoveImageFromCache(url); // Kaze
  //~ AddStyleToElement(gDialog.backgroundPreview, "background-image", backgroundImage);
  AddStyleToElement(gDialog.backgroundPreview, "background-image", 'url(' + MakeAbsoluteUrl(url) + ')'); // Kaze
  ...
  var xRadio, yRadio;
  // <Kaze> small changes + bugfix (yRadio detection)
  ...
  if (yBackgroundPosition == "100%" || yBackgroundPosition == "bottom")
    yRadio = "bottom";
  else if (yBackgroundPosition == "50%" || yBackgroundPosition == "center")
    yRadio = "center";
  else
    yRadio = "top";
...
// * Inits the values shown in the List tab panel
function InitListTabPanel()
{
  gDialog.selectedTab = LIST_TAB;
  if (!gDialog.selectedObject || !gDialog.selectedObject.style)
    return;
    
  SelectsOneChoice("list-style-type",     "listStyleTypeMenulist",     "ListStyleType",     null);
  SelectsOneChoice("list-style-position", "listStylePositionMenulist", "ListStylePosition", null);

  var listStyleImage = getSpecifiedStyle("list-style-image");
  gDialog.listStyleImageInput.setAttribute("value", listStyleImage);
}
...
function getSpecifiedStyle(property)
{
  if (gDialog.selectedObject && gDialog.selectedObject.style) {
    ...
    // <Kaze> keep URLs case-sensitives
    var value = gDialog.selectedObject.style.getPropertyValue(property);
    if (!(/^url\(/).test(value))
...
function SerializeExternalSheet(sheet, href)
{
  ...
  try {
    ...
    // CSS files should not begin with a comment, see: http://sourceforge.net/support/tracker.php?aid=2686924
    //~ var cssText = "/* Generated by CaScadeS, a stylesheet editor for Mozilla Composer */\n\n";
    //~ var cssText = "/* Generated by KompoZer */\n"; // Kaze ;-)
    var cssText = "";
...
function ToggleFourBorderSidesSameStyle(elt)
{
  ...
  for (var i=0; i<sideArray.length; i++) {
    ...
    if (sameStyle) {
      ...
      widthButton.setAttribute("hidden", "true"); // Kaze
      //~ colorInput.setAttribute("disabled", "true"); // Kaze
...
//   param String id
function Spinbutton(increment, id, defaultVal) 
{ // Kaze: added an option to specifiy a defaut value + adapted the increment values for some units
  var elt = document.getElementById(id);
  if (elt) {
    //elt.focus(); // Kaze: bugfix
    var value = elt.value;
    var re = /([+-]?\d*\.\d+|[+-]?\d+)(\D*)/ ;
    var found = value.match(re);
    if (found && found.length != 0) {
      ...
      elt.value = defaultVal ? defaultVal : "1px"; // Kaze
      //~ elt.value = increment + "px";
    try {
      elt.oninput();
    } catch(e) {
      // horrible hack: 'elt.oninput()' doesn't work any more on gecko 1.8+
      // if the corresponding element hasn't been manually focused before
      // (and the 'elt.focus()' stuff doesn't prevent this)
      eval(elt.getAttribute("oninput").replace('this', 'document.getElementById("' + id + '")'));
    }
  }
}
