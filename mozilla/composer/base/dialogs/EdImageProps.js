...
function Startup()
{
  ...
  gDialog.imageDimensionsTab = document.getElementById("imageDimensionsTab");
  gDialog.imageAppearanceTab = document.getElementById("imageAppearanceTab");
  gDialog.IsMailAddress      = document.getElementById("IsMailAddress");

  gDialog.alignTopOption     = document.getElementById("align-top-option");
  gDialog.alignMiddleOption  = document.getElementById("align-middle-option");
  gDialog.alignBottomOption  = document.getElementById("align-bottom-option");
  gDialog.alignLeftOption    = document.getElementById("align-left-option");
  gDialog.alignRightOption   = document.getElementById("align-right-option");
  ...
  if (tagName == "img" && IsCSSDisabledAndStrictDTD())
  {
  ...
  if (gOriginalHref.substr(0,7) == kHrefStr)
  {
    gDialog.IsMailAddress.checked = true;
    gDialog.hrefInput.value = gOriginalHref.substr(7);
  }
...
function InitDialog()
{
  ...
  {
    gDialog.imageAppearanceTab.setAttribute("collapsed", true);
    gDialog.showLinkBorder.setAttribute("collapsed", true);
  }

  if (IsStrictDTD())
  {
    gDialog.alignTopOption.setAttribute("collapsed", true);
    gDialog.alignMiddleOption.setAttribute("collapsed", true);
  }
}
...
{
  openHelp('comp-doc-image-edit','chrome://help/locale/nvu.rdf');
  return true;
}
  ...
  if (ValidateData())
  {
    ...
    try
    {
      ...
      if (href)
        ...
        {
          if (gDialog.showLinkBorder.checked)
            editor.setAttributeOrEquivalent(globalElement, "border", "2", true);
          else
            editor.setAttributeOrEquivalent(globalElement, "border", "0", true);
        }
        else
          editor.removeAttributeOrEquivalent(globalElement, "border", true);
