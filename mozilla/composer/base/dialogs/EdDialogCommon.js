...
function CenterWindowLocation()
{
  window.screenX = Math.max(0, Math.min(window.opener.screenX + (window.opener.outerWidth / 2)
                                                              - (window.document.width / 2),
                                        screen.availWidth - window.document.width ));
  window.screenY = Math.max(0, Math.min(window.opener.screenY + (window.opener.outerHeight / 2)
                                                              - (window.document.height / 2),
                                        screen.availHeight - window.document.height));
}
...
{
  return !node || cleanToLowerCase(node.localName) == 'br' || editor.nodeIsBlock(node);
}
...
function InsertElementAroundSelection(element)
{
  ...
  try {
    ...
    if (editor.nodeIsBlock(element))
      ...
      while (!(cleanToLowerCase(range.commonAncestorContainer.localName) in IsBlockParent))
    ...
    else
    {
      ...
      if (!nodeIsBreak(editor, range.commonAncestorContainer))
        return false;
      else if (cleanToLowerCase(range.commonAncestorContainer.localName) in NotAnInlineParent)
        // Inline element parent must not be an invalid block
        do range.selectNode(range.commonAncestorContainer);
        while (cleanToLowerCase(range.commonAncestorContainer.localName) in NotAnInlineParent);
      else
    ...
    else
    {
      ...
      if (start && start.localName.toLowerCase() == 'br')
      {
        editor.deleteNode(start);
        editor.insertNode(start, element, element.childNodes.length);
        empty = false;
