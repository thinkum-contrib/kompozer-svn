...
function InitImage()
{
  ...
  var align = GetHTMLOrCSSStyleValue(globalElement, alignStr, "float");
  if (align)
...
function ValidateImage()
{
  ...
  if (IsCSSDisabledAndStrictDTD())
  {
    ...
    globalElement.removeAttribute(alignStr);
  }
  else
  {
    // spacing attributes
    ...
    switch ( gDialog.alignTypeSelect.value )
    {
      case "top":
      case "middle":
        // these two values have no equivalence for the float property
        try {
          editor.removeAttributeOrEquivalent(globalElement, alignStr, true);
        } catch (e) {}
        editor.setAttribute(globalElement, alignStr,
                            gDialog.alignTypeSelect.value);
        break;
      case "right":
      case "left":
        editor.setAttributeOrEquivalent( globalElement, alignStr,
