...
function BuildBulletStyleList()
{
  ...
  if (gListType == "ul")
  {
    ...
    SetElementEnabled(gDialog.BulletStyleList, !IsCSSDisabledAndStrictDTD());
  ...
  else if (gListType == "ol")
  {
    ...
    var isStrictDTD = IsStrictDTD();
    SetElementEnabled(gDialog.StartingNumberInput, !isStrictDTD);
    SetElementEnabled(gDialog.StartingNumberLabel, !isStrictDTD);
  ...
  if (globalElement)
  {
    ...
    if (gListType == "ul")
    {
      if (gBulletStyleType && gDialog.ChangeAllRadio.selected)
        try {
          editor.setAttributeOrEquivalent(globalElement, "type", gBulletStyleType, true);
        } catch (e) {}
      else
        try {
