...
function SetupTextEditorCommands()
{
  ...
  commandTable.registerCommand("cmd_cleanup",    nsCleanupCommand);
...
//-----------------------------------------------------------------------------------
function UpdateCommandControllerContext()
{
  var controller;
  if (gComposerJSCommandControllerID)
  {
    try { 
      controller = window.content.controllers.getControllerById(gComposerJSCommandControllerID);
      controller.setCommandContext(GetCurrentEditorElement());
    } catch (e) {}
  }
  if (gComposerWindowControllerID)
  {
    try { 
      controller = window.content.controllers.getControllerById(gComposerWindowControllerID);
      controller.setCommandContext(GetCurrentEditorElement());
    } catch (e) {}
  }
...
function goUpdateCommandState(command)
{
  try
  {
    ...
    switch (command)
    {
      ...
      case "cmd_glues":
      case "cmd_dtdStrictness":
      case "cmd_dtdStrictness2":
        break;
...
//-----------------------------------------------------------------------------------
var nsDummyStrictnessCommand =
{
  isCommandEnabled: function(aCommand, dummy)
  {
    return (IsDocumentEditable() && IsEditingRenderedHTML() && !IsStrictDTD());
  },
...
//-----------------------------------------------------------------------------------
var nsDummyStrictness2Command =
{
  isCommandEnabled: function(aCommand, dummy)
  {
    return ( IsDocumentEditable() && IsEditingRenderedHTML() &&
             (!IsStrictDTD() || GetCurrentEditor().isCSSEnabled) );
  },
...
function openFileInEditor(aCommand, newTab)
{
  ...
  if (fp.file && fp.file.path.length > 0) {
    ...
    editPage(fp.fileURL.spec, window, false, newTab);
  }
}
...
var nsOpenCommand =
{
  ...
  doCommand: function(aCommand)
  {
    openFileInEditor(aCommand, false);
  }
};

//-----------------------------------------------------------------------------------
var nsOpenInTabCommand =
{
  isCommandEnabled: function(aCommand, dummy)
  {
    return true;    // we can always do this
  ...
  doCommand: function(aCommand)
  {
    openFileInEditor(aCommand, true);
  }
};
...
function PromptForSaveLocation(aDoSaveAsText, aEditorType, aMIMEType, aDocumentURLString)
{
  ...
  if (aDoSaveAsText)
    ...
    fp.appendFilter(GetString("HTMLtemplates"), "*.mzt");
  else if (IsXHTMLDocument())
    fp.appendFilter(GetString("XHTMLfiles"), "*.xhtml");
  else
    fp.appendFilters(nsIFilePicker.filterHTML);
  fp.appendFilters(nsIFilePicker.filterAll);
...
var gEditorOutputProgressListener =
{
  onStateChange : function(aWebProgress, aRequest, aStateFlags, aStatus)
  {
    ...
    if (aStateFlags & nsIWebProgressListener.STATE_STOP)
    {
      ...
      {
        try {
          gSiteManagerNotificationHandler(requestSpec, aStatus);
          ...
          && gPersistObj.currentState == nsIWebBrowserPersist.PERSIST_STATE_FINISHED)
      {
        if (GetScheme(gPublishData.publishUrl) == "file")
        {
          ...
          gSiteManagerNotificationHandler(gPublishData.publishUrl, aStatus);
...
var nsPublishSettingsCommand =
{
  ...
  doCommand: function(aCommand)
  {
    if (GetCurrentEditor())
    {
      ...
      window.ok = window.openDialog("chrome://editor/content/EditorPublishSettings.xul","_blank",
                                    "chrome,close,titlebar,modal", junk);
      window.content.focus();
      return window.ok;
    }
    return false;
...
var nsRevertCommand =
{
  ...
  doCommand: function(aCommand)
  {
    ...
    if (promptService)
    {
      ...
      var result = 0; // Kaze: prompt only if document is modified
...
function RemoveImageFromCacheAndUpdateAttribute(aImageSrc)
{
  try {
    ...
    if (IOService)
    {
      ...
      value = MakeAbsoluteUrl(aImageSrc);
...
function ResetAttribute(aElement, aAttribute, aImageSrc)
{
  try {
    aElement.setAttribute(aAttribute, aImageSrc);
  }
  catch(e) {}
}
function ResetProperty(aElement, aProperty, aImageSrc, aPriority)
{
  try {
    aElement.style.setProperty(aProperty, aImageSrc, aPriority);
  }
  catch(e) {}
}

//-----------------------------------------------------------------------------------
var nsRefreshImagesCommand =
{
  isCommandEnabled: function(aCommand, dummy)
  {
    // looking if there's an image in the document or any background applied
    // is probably too expensive...
    //~ return true;
    return (!IsUrlAboutBlank(GetDocumentUrl())); // Kaze
  },
  ...
  doCommand: function(aCommand)
  {
    function resetBackground(aElement)
    {
      ...
      if (aElement)
      {
        ...
        if (style)
        {
          var backgroundImage = aElement.style.getPropertyValue(property);
          if (backgroundImage)
          {
            if (RegExp( /url\((.*)\)/ ).test(backgroundImage))
            {
              var dummy = RegExp.$1;
              backgroundImage = dummy;
            }
            var priority = aElement.style.getPropertyPriority(property);
            aElement.style.setProperty(property,"url(" + backgroundImage + "foobarfoo)", priority);
            RemoveImageFromCacheAndUpdateAttribute(backgroundImage);
            setTimeout(ResetProperty, 100, aElement, property, "url(" + backgroundImage + ")", priority);
            aElement.style.setProperty(property,"url(" + backgroundImage + "foobarfoo)", priority);
            RemoveImageFromCacheAndUpdateAttribute(backgroundImage);
            setTimeout(ResetProperty, 100, aElement, property, "url(" + backgroundImage + ")", priority);
          }
        ...
        if (aElement.firstChild)
          resetBackground(aElement.firstChild);
        if (aElement.nextSibling)
          resetBackground(aElement.nextSibling);
      }
    }

    // first let's clear the cache
    //Components.classes["@mozilla.org/image/cache;1"].getService(Components.interfaces.nsIObserver).observe(null, "memory-pressure", null);

    // get all images
    var document = GetCurrentEditor().document;
    ...
    for (i = 0; i<imageList.length; i++)
    {
      ...
      RemoveImageFromCacheAndUpdateAttribute(src);
      setTimeout(ResetAttribute, 10, image, "src", src);
    }

    // update the body's background if any
    try {
      var bodyelement = GetBodyElement();
      if (bodyelement && bodyelement.hasAttribute("background"))
      {
        var background = bodyelement.getAttribute("background");
        bodyelement.removeAttribute("background");
        RemoveImageFromCacheAndUpdateAttribute(background);
        setTimeout(ResetAttribute, 100, bodyelement, "background", background);
      }
    } catch (e) {}

    // now let's browse all the style attributes...
    resetBackground(GetBodyElement());

    // let's do better now... We're going to refresh the backgrounds defined
    // by CSS stylesheets :-)

    function resetBackgroundInStylesheet(aSheet)
    {
      function resetBackgroundInStyleRuleList(aRuleList)
      {
        const property = "background-image";
        var i;
        for (i = 0; i < aRuleList.length; i++)
        {
          var rule = aRuleList[i];
          var type = rule.type;
          switch (type) {
            case CSSRule.STYLE_RULE:
              ...
              if (style)
              {
                var backgroundImage = rule.style.getPropertyValue(property);
                if (backgroundImage)
                {
                  if (RegExp( /url\((.*)\)/ ).test(backgroundImage))
                  {
                  ...
                  rule.style.setProperty(property,"url(" + backgroundImage + "foobarfoo)", priority);
                  RemoveImageFromCacheAndUpdateAttribute(backgroundImage);
                  setTimeout(ResetProperty, 100, rule, property, "url(" + backgroundImage + ")", priority);
                  rule.style.setProperty(property,"url(" + backgroundImage + "foobarfoo)", priority);
                  RemoveImageFromCacheAndUpdateAttribute(backgroundImage);
                  setTimeout(ResetProperty, 100, rule, property, "url(" + backgroundImage + ")", priority);
                }
            ...
            case CSSRule.IMPORT_RULE:
              resetBackgroundInStylesheet(rule.styleSheet);
              break;
            case CSSRule.MEDIA_RULE:
              resetBackgroundInStyleRuleList(rule.cssRules);
              break;
      ...
      resetBackgroundInStyleRuleList(aSheet.cssRules);
    }

    var stylesheets = document.styleSheets;
    for (i = 0; i < stylesheets.length; i++)
    {
      resetBackgroundInStylesheet(stylesheets[i]);
    }
  }
};

// <Kaze> these are needed for the context menu of the <tabeditor> component
function RevertTab(tab) { // revert the document in a tab
  if (!tab)
    return;

  var tabeditor = document.getElementById("tabeditor");
  if (tabeditor.selectedTab != tab) {
    tabeditor.selectedTab = tab;
    tabeditor.SelectTabEditor();
  }
  nsRevertCommand.doCommand();
}

function RefreshTab(tab) { // refresh images in a tab
  if (!tab)
    return;
...
var nsCloseCommand =
{
  ...
  doCommand: function(aCommand)
  {
    ...
    UpdateWindowTitle(); // Kaze
  }
};

//-----------------------------------------------------------------------------------
var nsCloseAllCommand =
{
  isCommandEnabled: function(aCommand, dummy)
  {
    return GetCurrentEditor() != null;
...
function CloseCurrentTab(aDontLeaveTabeditorEmpty)
{
  var handled = CheckAndSaveDocument("cmd_close", true);
  if (handled) 
  {
    var tabeditor = document.getElementById("tabeditor");
    var tab    = tabeditor.selectedTab;
    var index  = tabeditor.selectedIndex;
    var n      = tabeditor.mTabpanels.childNodes.length;
    var editor = tabeditor.selectedPanel;

    var newIndex = Math.min(index, n - 2);

    editor.parentNode.removeChild(editor);
    tab.parentNode.removeChild(tab);
    if (newIndex >= 0)
      tabeditor.selectedIndex = newIndex;
    else if (aDontLeaveTabeditorEmpty)
    {
      var prefs = GetPrefs();
      ...
      tabeditor.newBlankTab(preferredDoctype, strictness);
    }
  }
  return handled;
}
...
function CloseTab(tab)
{
  ...
  var tabeditor = document.getElementById("tabeditor");
  var currentTab = tabeditor.selectedTab;
  var currentIndex = tabeditor.selectedIndex;
  var n  = tabeditor.mTabpanels.childNodes.length;

  var newIndex = Math.min(currentIndex, n - 2);
  ...
  if (handled) 
  {
    ...
    {
      if (tab != currentTab)
        tabeditor.selectedTab = currentTab;
      else
        tabeditor.selectedIndex = newIndex;
    }
    else
...
function RemoveAllTabsBut(tabToKeep)
{
  if (!tabToKeep)
    return;

  var tabeditor = document.getElementById("tabeditor");
  var tabs      = tabeditor.mTabs;
  var tab       = tabs.lastChild;
  while (tab)
  {
    var tmp = tab.previousSibling;
    if (tabToKeep != tab)
    {
      CloseTab(tab);
    }
...
var nsNewEditedObjectCommand =
{
  ...
  doCommand: function(aCommand)
  {
      /* The last parameter is the current browser window.
         ...
         and loading into existing browser option is removed
       */
      window.openDialog( "chrome://communicator/content/newDocument.xul", "_blank", "chrome,modal,titlebar", 0);
    window.content.focus();
  }
};
...
var nsPrintCommand =
{
  ...
  doCommand: function(aCommand)
  {
    ...
    try {
      // NSPrint();
      var webBrowserPrint = GetCurrentEditorElement().contentWindow
...
var nsCleanupCommand =
{
  ...
  {
    try {
      window.openDialog("chrome://editor/content/MarkupCleaner.xul", "_blank",
...
var nsValidateCommand =
{
  ...
  doCommand: function(aCommand)
  {
    ...
    if (ifile)
    {
      window.openDialog("chrome://editor/content/Validator.xul", "_blank", "chrome,close,titlebar,resizable=yes", URL2Validate, true);
    }
    else
    {
      window.openDialog("chrome://editor/content/Validator.xul", "_blank", "chrome,close,titlebar,resizable=yes", URL2Validate, false);
    }
  },
  ...
  doCommand: function(aCommand)
  {
    ...
    else
    {
      try {
        ...
        {
          editor.removeAttributeOrEquivalent(hLine, "align", true);
          editor.removeAttributeOrEquivalent(hLine, "width", true);
          editor.removeAttributeOrEquivalent(hLine, "size", true);
          editor.removeAttributeOrEquivalent(hLine, "noshade", true);
        }
        else
        {
...
//-----------------------------------------------------------------------------------
var nsInsertPHPorCommentCommand =
{
  isCommandEnabled: function(aCommand, dummy)
  {
    return (IsDocumentEditable() && IsEditingRenderedHTML());
  ...
  doCommand: function(aCommand)
  {
    //~ window.openDialog("chrome://editor/content/EdPHPorPI.xul","_blank", "chrome,close,titlebar,modal", "", null, aCommand);
    window.openDialog("chrome://editor/content/EdPHPorPI.xul","_blank", "chrome,close,titlebar,modal,resizable=yes", "", null, aCommand); // Kaze
    window.content.focus();
  }
};
...
var nsObjectPropertiesCommand =
{
  ...
  doCommand: function(aCommand)
  {
    ...
    if (element)
    {
      ...
      if (element.namespaceURI == NVU_NS)
      {
        EditPIorComment(element);
      }
      else
      {
        switch (name)
...
var nsGluesCommand =
{
  isCommandEnabled: function(aCommand, dummy)
  {
    ...
    if (e)
    {
      e instanceof Components.interfaces.nsIHTMLAbsPosEditor;
      var elt = e.positionedElement;
      if (elt)
        return true;
    }
    return false;
  ...
  doCommandParams: function(aCommand, aParams, aRefCon)
  {
    var glueDirection = aParams.getCStringValue("state_attribute");
    var e = GetCurrentEditor();
    e instanceof Components.interfaces.nsIHTMLAbsPosEditor;
    var elt = e.positionedElement;
    ...
    var xuldoc = editorElt.ownerDocument;
    var cx = xuldoc.defaultView.getComputedStyle(editorElt , "");
    var docHeight = cx.getPropertyCSSValue("height").getFloatValue(CSSPrimitiveValue.CSS_PX);
    var docWidth = cx.getPropertyCSSValue("width").getFloatValue(CSSPrimitiveValue.CSS_PX);

    // WARNING : we deal only with elements positioned relatively to the viewport, not
    // to other positioned elements.
    
    switch (glueDirection)
    {
      case "left-pixels":
        newStyle = "left: " + elt.offsetLeft + "px; right: auto;";
        break;
      case "right-pixels":
        newStyle = "right: " + (docWidth - elt.offsetLeft - elt.realWidth)+ "px; left: auto;";
        break;
      case "center-pixels":
        var rpx = elt.offsetLeft;
        var rpc = (rpx / docWidth) * 100;
        newStyle = "left: " + rpc + "%; right: auto;";
        break;

      case "top-pixels":
        newStyle = "top: " + elt.offsetTop + "px; bottom: auto;";
        break;
      case "bottom-pixels":
        newStyle = "bottom: " + (docHeight - elt.offsetTop - elt.realHeight) + "px; top: auto;";
        break;
      case "middle-pixels":
        var mpx = elt.offsetTop;
        var mpc = (mpx / docHeight) * 100;
        newStyle = "top: " + mpc + "%; bottom: auto;";
        break;
    ...
    if (newStyle)
    {
      ...
      e.setAttribute(elt, "style", style + newStyle);
    }
  },
...
var nsEditLinkCommand =
{
  ...
  doCommand: function(aCommand)
  {
    try {
      ...
      if (element)
        editPage(element.href, window, false, true);
    } catch (e) {}
    window.content.focus();
  }
};
...
var nsBlockBordersCommand =
{
  ...
  doCommand: function(aCommand)
  {
    window.openDialog("chrome://cascades/content/borderProps.xul","_blank", "chrome,close,modal,titlebar",
                      "border", "Borders", true);
    window.content.focus();
  }
};
